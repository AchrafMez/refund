...
function CustomCalendar({ value, onChange, onClose }: CustomCalendarProps) {
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px 0' }}>
        ...
        {days.map((day) => {
          ...
          if (value.start) {
            if (currentDate.toDateString() === value.start.toDateString()) {
              isSelected = true
              isStart = true
            }
          }
          if (value.end) {
            if (currentDate.toDateString() === value.end.toDateString()) {
              isSelected = true
              isEnd = true
            }
          }
          if (value.start && value.end) {
            if (currentDate > value.start && currentDate < value.end) {
              isRange = true
            }
          }

          // Determine background for range
          let cellBg = '#ffffff'
          if (isRange) cellBg = '#f4f4f5'
          if (isStart && value.end) cellBg = 'linear-gradient(to right, #ffffff 50%, #f4f4f5 50%)'
          if (isEnd && value.start) cellBg = 'linear-gradient(to left, #ffffff 50%, #f4f4f5 50%)'
      ...
      <div style={{ marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid #f4f4f5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: '0.75rem', fontWeight: 500, color: '#3f3f46', marginTop: '0.125rem' }}>
            {value.start ? formatDate(value.start) : "Start"}
            {value.start && value.end ? ` – ${formatDate(value.end)}` : ""}
          </span>
        </div>
